...
目前已知国内外钢管圆度检测方法主要分为两大类，分别是接触式测量和非接触测量。
传统方法中，采用用拉线法与直尺法，工人通过拉线或直尺直接接触钢管表面，肉眼判断直线度偏差。
...
早在2001年便有提到一种基于机器视觉的钢管直度检测方案\cite{cv}，然而该方案受限于视觉传感器的像素精度。
2002年Wei Gao团队提到了一种使用三对单点激光测距进行钢管直度检测的方案，使用了差分积分法能够有效消除由于传感器安装位置导致的误差，
并且提出了一种传感器调零校准的方案\cite{pm}。但是单点测距的范围较小，且基于检测钢管外轮廓的方案难以避免由于钢管左右扭动或运动偏心造成的误差。 
...
到了2020年，宝山钢铁有限公司的张建伟团队将非接触式激光测量与全周旋转技术相结合，显著提升了钢管管端直线度误差的测量精度和效率。
这种技术能够很好地满足现代化生产的高要求，但其高昂的设备成本和复杂的数据处理流程可能限制其在某些场景下的应用推广。
...
\begin{itemize}
    \item RGBD相机坐标系（$R_1$、$R_2$）：$x$为相机光轴方向，$y$轴为相平面长边方向。
...
由于RGBD相机的可见局部点云，点沿轴线方向的散布远大于横截面方向的散布，因此最大方差方向必为轴线方向。寻找真实轴线问题转换为寻找一组三维正交基$\{\mathbf{e}_1, \mathbf{e}_2, \mathbf{e}_3\}$使得点云数据在这些正交基上的投影方差依次达到最大，这等价于对局部点云进行主成分分析（PCA）。
...
RANSAC的作用是从含有离群点的点集中，快速估计一个较可靠的圆初值。每次迭代随机选择三个非共线点 $\mathbf{p}_a,\ \mathbf{p}_b,\ \mathbf{p}_c$，三点唯一确定一个候选圆 $\hat{\mathbf{c}},\ \hat{r}$。对所有点计算候选模型残差，若残差小于阈值 $\tau$，则该点被认为是内点：
\begin{equation}
...
对上式求L2范数，便可以得到对应点消除了所有旋转平移误差的曲率：
\begin{equation}
...
\subsection{微缩测试架设计}
为验证本团队提出的非接触式钢管直线度检测方案的可行性，并解决大尺寸钢管在实际检测中面临的固定、传送与数据采集难题，
研发了本套钢管直度检测实验装置。该装置旨在模拟检测环境，为后续算法的验证与优化提供稳定、可靠的硬件平台及高质量的数据源。

\subsubsection{设计目标和装置参数}
本装置的核心设计目标在于：在确保待测钢管被完全固定的条件下，通过一套电机驱动的精密直线运动模块，带动实感深度相机沿钢管轴向进行匀速、
平稳的扫描运动，从而连续、精确地采集钢管表面的三维深度信息，最终生成可用于后续算法处理的高质量点云数据。

我们的检测对象为长度2000mm、外径200mm、壁厚2mm的仿真钢管模型。要求在整个检测过程中，待测钢管只得发生有限的轴向移动或径向滚动，
不得超出给定相机范围，用以测试。驱动相机移动的机构运行需平稳，确保数据采集过程中相机无显著抖动或晃动，以保证采集数据的一致性。

\subsubsection{装置概述}

为实现上述目的，我们采用“伺服电机/步进电机+同步带”的传动方案，驱动安装有深度相机的滑台，
沿高精度直线导轨做双向、匀速运动，实现对钢管外表面的自动化、全覆盖式扫描。
并设计结构提供可靠的支撑与夹持机构，确保检测对象在整个测量过程中位置绝对固定。

\subsubsection{结构设计}

装置主体框架采用国标4040铝型材搭建。该材料具备轻质、高强、耐腐蚀、易于加工与装配的优点，非常适合用于快速构建实验原型设备。
此框架结构为整个装置提供了必要的刚性与整体稳定性，是各运动模块可靠的安装基础。系统总体结构如图\ref{fig:mechanical_structure_overview}

\begin{figure}[!htb]
	\centering
	\includegraphics[width=\linewidth]{images/mechanical_structure_overview.png}
	\caption{机械结构总览}
...
装置主要由以下功能组件构成：
\begin{itemize}
\item 固定支架：如图\ref{fig:mechanical_structure_fixed_support}采用V型块或定制夹具，用于承载并锁紧仿真钢管模型，
      完全满足技术指标中对钢管约束的要求。
\item 直线运动模块：包含高负载线性滑轨、滑块、同步带、驱动轮、从动轮及张紧机构。相机安装板与滑块刚性连接，
      由同步带驱动实现精确的直线往复运动。滑块部分如图\ref{fig:mechanical_structure_slide}所示。
\item 驱动单元：采用伺服电机或步进电机，通过专用的电机安装板固定于铝型材框架端部，为系统提供动力。电机与运动控制器连接，
      可精确控制扫描速度、启停位置及运动行程。
\item 相机安装板：定制化的安装板件，用于牢固固定实感深度相机。板上可设计调节机构，用于微调相机的高度和俯仰角度，
      确保其光轴精确对准钢管中心轴线。
\end{itemize}
...
基于仿真模型，我们对上述算法进行了验证，结果如图\ref{fig:results}所示。仿真钢管长度10\,m，弯曲角度10°，理论端点间弓高为0.1964\,m。
...
\textbf{传感器精度有限。}系统采用Intel D435消费级RGBD相机，深度测量噪声约为$\sigma \approx 2\,\text{mm}$，远高于工业直度检测的毫米级精度要求。在当前硬件条件下，算法的理论精度上限受限于传感器本身，测试结果仅具参考意义。

\textbf{实物验证未按期完成。}受限于开发进度，圆拟合、二阶差分、分Bin平均及曲率积分等核心算法均未在真实硬件平台上完成端到端验证，目前仅完成了点云分割算法的实物验证。

\begin{thebibliography}{99}
	\bibitem{cv} Lu R S, Li Y F, Yu Q. On-line measurement of the straightness of seamless steel pipes using machine vision technique[J]. Sensors and Actuators A: Physical, 2001, 94(1-2): 95-101.
    \bibitem{pm} Gao W, Yokoyama J, Kojima H, et al. Precision measurement of cylinder straightness using a scanning multi-probe system[J]. Precision Engineering, 2002, 26(3): 279-288.
	\bibitem{hxf} 黄雄飞, 张乾峰, 张全宝, 等. 直缝钢管直度测量方法的探讨[J]. 焊管, 2017, 40(5): 41-43.
	\bibitem{fast_lio} Xu W, Cai Y, He D, et al. Fast-lio2: Fast direct lidar-inertial odometry[J]. IEEE Transactions on Robotics, 2022, 38(4): 2053-2073.
	\bibitem{fz} Pang C, Wang J, Liu Z, et al. Robust LiDAR-IMU-GNSS fusion pose estimation with the point cloud registration uncertainty modeling[J]. Measurement, 2025: 119424.
\end{thebibliography}
