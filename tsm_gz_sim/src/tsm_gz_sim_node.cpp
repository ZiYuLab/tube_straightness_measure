    ...
    : Node("tsm_gz_sim_node", options), tf_broadcaster_(this)
{
  ...
  tube_model_name_ =
      declare_parameter<std::string>("tube_model_name", "straight_tube");
  tf_frame_id_ = declare_parameter<std::string>("tf_frame_id", "world");
  tf_child_frame_id_ =
      declare_parameter<std::string>("tf_child_frame_id", "tube");
  ...
  std::filesystem::create_directories("/tmp/tsm_sim");
  pose_log_.open("/tmp/tsm_sim/pose_log.csv");
  pose_log_ << "elapsed_s,x,y,z,angle\n";
...
void TsmGzSimNode::onTimer()
{
  ...
  double vib_y =
      vib_amplitude_ * std::sin(2.0 * M_PI * vib_frequency_ * elapsed_s_);
  double vib_z =
      vib_amplitude_ *
      std::sin(2.0 * M_PI * vib_frequency_ * elapsed_s_ + M_PI / 3.0);
  ...
  double angle = tilt_angle_ + drift_angle_ * t +
                 tilt_vib_amplitude_ *
                     std::sin(2.0 * M_PI * tilt_vib_frequency_ * elapsed_s_);
  ...
  if (pose_log_.is_open())
    pose_log_ << elapsed_s_ << "," << x_pos_ << "," << vib_y << ","
              << (vib_z + drift_z_ * t) << "," << angle << "\n";
}
